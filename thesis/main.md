...
第二类是算法层面的方法。这类方法通过修改损失函数或训练策略来平衡多数类和少数类的梯度贡献。典型方法包括Focal Loss、Class-Balanced Loss、LDAM（Label-Distribution-Aware Margin Loss）等。Focal Loss通过降低易分类样本的权重，使得模型更关注难分类样本；Class-Balanced Loss为每个类别设置与其样本数成反比的权重，平衡各类别的梯度贡献；LDAM为少数类设置更大的分类边界，强制模型在少数类上学习更鲁棒的特征表示。然而，本研究尝试Focal Loss后，模型性能完全崩溃，Macro-F1仅为0.09。深入分析表明，在样本比达1:146的极端不均衡场景下，Focal Loss的优化方向可能完全偏离，导致模型无法学习到有效的特征表示。算法层面方法虽然理论上合理，但在实际应用中往往难以取得理想效果，特别是在极端不均衡场景（如1:146的样本差距）下。
